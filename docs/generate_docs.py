"""
Generate documentation files for the OpenAD documentation website.
...
website_repo_docs = os.path.normpath(os.path.join(dir, "../../openad-website/main/docs"))
...
############################################################
# region - openad-toolkit -- README.md


# Update the main README.md with OpenAD description
def update_readme_md(filename="README.md"):
    output_text(f"<h1>Updating <yellow>{filename}</yellow> with OpenAD description</h1>", pad_top=2)

    file_path = os.path.normpath(os.path.join(dir, "..", filename))

    # Read README.md file content
    readme_md, err_msg = open_file(file_path, return_err=True)
    if not readme_md:
    ...
    description_txt, err_msg = open_file(os.path.join(dir, "source", "description.txt"), return_err=True)
    ...
    # Insert description
    readme_md = re.sub(
        r"<!-- autogenerated:description -->.*<!-- /autogenerated:description -->",
        f"<!-- autogenerated:description -->\n{description_txt.strip()}\n<!-- /autogenerated:description -->",
        readme_md,
        flags=re.DOTALL,
    )
...
############################################################
# region - openad-website -- docs/plugins.md


# Update the docs/plugins.md file with the about_plugins description
def update_docs_plugins_md(filename="plugins.md"):
    output_text(f"<h1>Updating <yellow>{filename}</yellow> with about_plugins description</h1>", pad_top=2)

    file_path_src = os.path.join(website_repo_docs, filename)
    file_path_output = os.path.join(dir, "output", "markdown", filename)

    # Read README.md file content
    readme_plugin_md, err_msg = open_file(file_path_src, return_err=True)
    if not readme_plugin_md:
        output_text(FLAG_ERROR, pad_top=1)
        output_error(err_msg)
        return

    # Read about_plugins source file content
    about_plugins_txt, err_msg = open_file(os.path.join(dir, "source", "about_plugins.txt"), return_err=True)
    if not about_plugins_txt:
        output_text(FLAG_ERROR, pad_top=1)
        output_error(err_msg)
        return

    # Insert about_plugins text
    readme_plugin_md = re.sub(
        r"<!-- autogenerated:plugins -->.*<!-- /autogenerated:plugins -->",
        f"<!-- autogenerated:plugins -->\n{about_plugins_txt.strip()}\n<!-- /autogenerated:plugins -->",
        readme_plugin_md,
        flags=re.DOTALL,
    )

    # Write to output file
    success, err_msg = write_file(file_path_output, readme_plugin_md, return_err=True)
    if success:
        output_text(FLAG_SUCCESS)
...
def update_docs_base_concepts_md(filename="base-concepts.md"):
    ...
    base_concepts_md, err_msg = open_file(file_path_src, return_err=True)
    ...
    about_mws_txt, err_msg = open_file(os.path.join(dir, "source", "about_mws.txt"), return_err=True)
    ...
    about_workspaces_txt, err_msg = open_file(os.path.join(dir, "source", "about_workspaces.txt"), return_err=True)
    ...
    about_runs_txt, err_msg = open_file(os.path.join(dir, "source", "about_runs.txt"), return_err=True)
    ...
    # Insert descriptions
    base_concepts_md = re.sub(
        r"<!-- autogenerated:mws -->.*<!-- /autogenerated:mws -->",
        f"<!-- autogenerated:mws -->\n{about_mws_txt.strip()}\n<!-- /autogenerated:mws -->",
        base_concepts_md,
        flags=re.DOTALL,
    )
    base_concepts_md = re.sub(
        r"<!-- autogenerated:workspaces -->.*<!-- /autogenerated:workspaces -->",
        f"<!-- autogenerated:workspaces -->\n{about_workspaces_txt.strip()}\n<!-- /autogenerated:workspaces -->",
        base_concepts_md,
        flags=re.DOTALL,
    )
    base_concepts_md = re.sub(
    ...
    base_concepts_md = re.sub(
        r"<!-- autogenerated:runs -->.*<!-- /autogenerated:runs -->",
        f"<!-- autogenerated:runs -->\n{about_runs_txt.strip()}\n<!-- /autogenerated:runs -->",
        base_concepts_md,
        flags=re.DOTALL,
    )
    ...
    # Write to file
    success, err_msg = write_file(file_path_output, base_concepts_md, return_err=True)
    if success:
        output_text(FLAG_SUCCESS)
        output_text(f"<soft>Exported to</soft> <reset>/docs/output/markdown/{filename}</reset>")
    else:
...
def generate_commands_md(filename="commands.md"):
    ...
    file_path_src = os.path.join(dir, "input", filename)
    file_path_output = os.path.join(dir, "output", "markdown", filename)
    ...
    commands_md, err_msg = open_file(file_path_src, return_err=True)
    if not commands_md:
        output_text(FLAG_ERROR, pad_top=1)
        output_error(err_msg)
        return
    ...
    # Write to file:
    success, err_msg = write_file(file_path_output, commands_md, return_err=True)
...
def generate_commands_csv(filename="commands.csv", delimiter=";"):
    ...
    file_path_output = os.path.join(dir, "output", "csv", filename)
    ...
    # Write to file
    success, err_msg = write_file(file_path_output, output_str, return_err=True)
    if success:
        output_text(FLAG_SUCCESS)
...
if __name__ == "__main__":
    update_readme_md()
    update_docs_plugins_md()
    update_docs_base_concepts_md()
    generate_commands_md()
    generate_commands_csv()

    # Move all generated markdown files to the website repo
    output_text(
        "\n".join(
            [
                "Do you want to copy the generated markdown files to the website repo?",
    ...
    if ok:
        ...
        output_warning(
            "No files copied over to the website repo.\n<soft>You can still find them in the /docs/output directory</soft>"
        )
